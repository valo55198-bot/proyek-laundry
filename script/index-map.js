(function () {
    ...
    document.getElementById('btnGeolocate').addEventListener('click', function () {
        ...
        var _highAccFailed = false; // flag: apakah high-accuracy sudah gagal

        function startWatch(highAccuracy) {
            var opts = highAccuracy
                ? { enableHighAccuracy: true, timeout: 30000, maximumAge: 10000 }
                : { enableHighAccuracy: false, timeout: 20000, maximumAge: 30000 };

            if (!highAccuracy) {
                btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Mencari (WiFi)...';
            }

            // Batas waktu maksimal keseluruhan 90 detik
            ...
            _watchId = navigator.geolocation.watchPosition(
                function (pos) {
                    ...
                    // Update alamat jika akurasi membaik
                    if (acc < bestAccuracy) {
                        bestAccuracy = acc;
                ...
                function (err) {
                    ...
                    // Jika high-accuracy timeout → coba fallback ke low-accuracy (WiFi/IP)
                    if (err.code === 3 && highAccuracy && !_highAccFailed) {
                        _highAccFailed = true;
                        console.log('GPS high-accuracy timeout, fallback ke low-accuracy...');
                        startWatch(false); // coba lagi pakai WiFi/network
                        return;
                    }

                    var msg = 'Gagal mendapatkan lokasi.';
                    if (err.code === 1) msg = 'Akses lokasi ditolak. Izinkan di pengaturan browser.';
                    else if (err.code === 2) msg = 'Lokasi tidak tersedia. Pastikan GPS atau WiFi aktif.';
                    else if (err.code === 3) msg = 'Timeout. Coba aktifkan WiFi atau pindah ke tempat lebih terbuka.';
        ...
        startWatch(true); // mulai dengan high-accuracy
    });
